...
function showCopiedAlert() {
  ...
  if (!alertDiv) {
    ...
    alertDiv.style.marginTop = '6px';
    alertDiv.style.textAlign = 'center';
    copiedPreview.parentNode.insertBefore(alertDiv, copiedPreview.nextSibling);
  }
...
async function createActionButtons(snippet, index, isFavorite = false) {
  ...
  editDeleteDiv.className = 'edit-delete-actions';
...
async function createListItem(snippet, index, isFavorite = false) {
  ...
  textContainer.textContent = snippet.text;
  ...
  timestamp.title = new Date(snippet.copiedAt).toLocaleString(); // Keep full timestamp in tooltip
...
function updateCopiedPreview(snippets) {
  ...
  copiedPreview.textContent = `Copied: "${snippets[0].text.length > 80 ? snippets[0].text.slice(0, 80) + '…' : snippets[0].text}"`;
  copiedPreview.title = snippets[0].text;
}
